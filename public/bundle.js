(function () {
	...
	var template = "<div class=\"side-dashboard-component\"><header><div class=\"dashboard-logo\"></div></header><section><div class=\"employee-collection\"><employee-lite v-for=\"item in collection\" :active=\"view\" :model=\"item\" :key=\"item.key\" @inspect-item=\"handleClick\"></employee-lite></div></section></div>";

	var template$1 = "<div class=\"employee-lite-component\" :class=\"isActive\"><span :style=\"inlineFontSize\" @click=\"handleClick\">{{ model.label }}</span></div>";
	...
	var config = {
		...
		paths: {
			employees: "/_images/profile-pics"
		}
	};
	...
	var EmployeeLite = {
	    ...
	    computed: {
	        ...
	        inlineFontSize: function inlineFontSize() {
	        	var size = (CSS.MAX_FONT_SIZE / CSS.MAX_RANGE) * this.model.popularity; 
	        	return {
	        		fontSize: (size + "em")
	        	}
	        }
	...
	var SideDashboard = {
	    name: "side-dashboard",
	    template: template,
	    components: { EmployeeLite: EmployeeLite },
	...
	var template$2 = "<transition name=\"switch\" mode=\"in-out\"><div class=\"page-content-component\" v-if=\"modelUpdated\"><article><header><div class=\"box\"><div class=\"el-image\" :style=\"inlineBackgroundImage\"></div></div><div class=\"box\"><div class=\"el-title\"><span>{{ model.name }}</span></div><slider :value=\"model.popularity\" name=\"popularity\" min=\"1\" max=\"5\" @update-value=\"updatePopularity\"></slider></div></header><section><div class=\"article-body\"><h4>Biography</h4><p>{{ model.biography }}</p></div></section></article></div></transition>";

	var template$3 = "<div class=\"slider-component\"><span>{{ nameAsLabel }}</span><div class=\"field-input\"><label :for=\"name\"> </label><input :id=\"name\" type=\"range\" :min=\"min\" :max=\"max\" v-model=\"model\" @change=\"update\"></div></div>";
	...
	var Slider = {
	    ...
	    computed: {
	    	nameAsLabel: function nameAsLabel() {
	...
	function capitalise(word) {
		...
		var rest = word.toString().slice(1);
		return ("" + CapitalLetter + rest)
	}

	var IMG_URL = config.paths.employees;
	var PageContent = {
	    name: "page-content",
	    template: template$2,
	    ...
	    computed: {
	        inlineBackgroundImage: function inlineBackgroundImage() {
	            return {
	    ...
	    methods: {
	    	updatePopularity: function updatePopularity(value) {
	    		this.$emit("update-popularity", { key: this.model.key, value: value });
	    	}
	    },
	    watch: {
	...
	var App = {
		el: "#root",
		components: { SideDashboard: SideDashboard, PageContent: PageContent },
		data: {
			employees: [],
			view: null
		},
		...
		computed: {
			dashboardCollection: function dashboardCollection() {
				return this.employees.map(basicEmployeeData)
			},
			getCurrentTarget: function getCurrentTarget() {
				var this$1 = this;

				return this.view ? this.employees.filter(function (child) { return child.key === this$1.view; }).pop() : null
			},
			showPageContent: function showPageContent() {
				return this.view !== null
			}
		},
		methods: {
			setPageView: function setPageView(key) {
				this.view = key;
			},
			updatePopularity: function updatePopularity(ref) {
				var key = ref.key;
				var value = ref.value;
	...
	var addKeys = function (child) {
		var key = child.name.toLowerCase().split(" ").join("-");
		return Object.assign({}, child, {key: key})
	};
	var basicEmployeeData = function (child) { 
		return { key: child.key, label: child.name, popularity: child.popularity } 
	};

	window.App = new (Function.prototype.bind.apply( Vue, [ null ].concat( App) ));
